...
{
    public class IconaController : Controller
    {
        private const int IconaWidth = 400;  // ToDo - Change the size of the stored avatar image
        private const int IconaHeight = 400; // ToDo - Change the size of the stored avatar image
        private const int IconaScreenWidth = 900;  // ToDo - Change the value of the width of the image on the screen

        private const string TempFolder = "/Temp";
        private const string MapTempFolder = "~" + TempFolder;
        private const string IconaPath = "/Content/Immagini/Icona";

        private readonly string[] _imageFileExtensions = { ".jpg", ".png", ".gif", ".jpeg" };

        [HttpGet]
        public ActionResult UploadIcona(int? id)
        {
            ViewBag.Eid = id;
            return View();
        }
        ...
        [HttpGet]
        public ActionResult _UploadIcona()
        {
            return PartialView();
        }

        [ValidateAntiForgeryToken]
        public ActionResult _UploadIcona(IEnumerable<HttpPostedFileBase> files)
        {
        ...
        [HttpPost]
        public ActionResult Save(int eid, string t, string l, string h, string w, string fileName)
        {
            try
            {
                ...
                img.Crop(top, left, img.Height - top - IconaHeight, img.Width - left - IconaWidth);
                // ... delete the temporary file,...
                System.IO.File.Delete(fn);
                // ... and save the new one.
                var newFileName = Path.Combine(IconaPath, eid + extension);
        ...
        private static string SaveTemporaryAvatarFileImage(HttpPostedFileBase file, string serverPath, string fileName)
        {
            ...
            img.Resize(IconaWidth, (int)(IconaWidth * ratio));
